...
def start(update, context):
    ...
    if len(text) <= 7:
        ...
        try:
            ...
            print(f'{username} just linked their account.')
...
def getpic(r, uid, context):
    ...
    try:
        pfp  = context.bot.getUserProfilePhotos(uid)['photos'][-1][0]['file_id']
        user = requests.get(context.bot.getFile(pfp).file_path)
    except:
        user = requests.get('https://files.catbox.moe/ofq4w4.jpg')
